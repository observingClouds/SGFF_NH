...
ds_in_dropped_NaTs.to_netcdf(output_file,
                              encoding={"counts":
                                        {'zlib':True, 'dtype':'int','_FillValue':0},
                                        "time":
                                        {'dtype':'object'},
                                        "class":
                                        {'_FillValue':None}
...
ds_in2.counts.attrs['long_name'] = 'Number of cells with a particular pattern within each 1deg x 1deg box'
ds_in2['latitude_bnds'] = xr.DataArray(np.array([np.arange(-10,54,1),np.arange(-9,55,1)]).T, dims=['latitude', 'bnds'])
ds_in2['longitude_bnds'] = xr.DataArray(np.array([np.arange(-100,9,1),np.arange(-99,10,1)]).T, dims=['longitude', 'bnds'])
...
ds_in2.to_netcdf(output_file+'2',
                             encoding={"counts":
                                       {'zlib':True, 'dtype':'int','_FillValue':0},
                                       "time":
                                       {'dtype':'object'},
