...
def download_MODIS_imgs(year_range, months, save_path, lon_range, lat_range,
                  deg2pix=100, satellite='Aqua', exist_skip=False, var='CorrectedReflectance_TrueColor',filetype='jpeg'):
        ...
        for m in months:
            ...
            for nd in range(1,nday+1):
                ...
                url = ('https://wvs.earthdata.nasa.gov/api/v1/snapshot?REQUEST=GetSnapshot&TIME='+
                       date.strftime('%Y-%m-%d')+loc+'&CRS=EPSG:4326'+
                       layer+'&FORMAT=image/'+filetype+
                       size)
